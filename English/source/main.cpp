 ...
 * Red-J wrote this file. As long as you retain this notice you
...
////////////////////////////////////////////
	...
	texteCROSS1 = TTF_RenderText_Blended(font, "INStALL FILES", colorwhite);// déclaration de la variable texteCROSS1
	texteSQUARE1 = TTF_RenderText_Blended(font, "REStore FILES", colorwhite);// déclaration de la variable texteSQUARE1
	texteTRIANGLE1 = TTF_RenderText_Blended(font, "ABOUt", colorwhite);// déclaration de la variable texteTRIANGLE1
	texteCIRCLE1 = TTF_RenderText_Blended(font, "QUIt", colorwhite);// déclaration de la variable texteCIRCLE1
	texteUse = TTF_RenderText_Blended(font, "PUt YOUR CUStom FILES IN  ", colorwhite);// déclaration de la variable texteUse
	texteUse2 = TTF_RenderText_Blended(font, "'custom' FOLDER ON YOUR USB KEY", colorwhite);// déclaration de la variable texteUse
	about = TTF_RenderText_Blended(font2, "ABOUt", colorwhite);// déclaration de la variable texteUse
	about2 = TTF_RenderText_Blended(font, "HOMEBREW BUILD WItH PSL1GHt V2", colorwhite);// déclaration de la variable texteUse
	about3 = TTF_RenderText_Blended(font, "tHANKS tO ALL MEMBERS WHO MOVE ", colorwhite);// déclaration de la variable texteUse
	about4 = TTF_RenderText_Blended(font, "tHE PS3 SCENE AND FRIENDS COOL72 ", colorwhite);// déclaration de la variable texteUse
	about5 = TTF_RenderText_Blended(font, "LIttLEBALUP MARKUS95 ALDO AND MORE", colorwhite);// déclaration de la variable texteUse
	about6 = TTF_RenderText_Blended(font, "tHE ONLY DEV LIMIt IS HIS IMAGINAtION...", colorwhite);// déclaration de la variable texteUse
	...
	SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 255, 255, 255)); 
		...
		position.x = 220; //position horizontale de	texteUse								
		...
		position.x = 120; //position horizontale de	texteUse2								
				...
				if(joypadInfo.port_status[0])
				{
					ioPadGetData(0, &joypadData);
						if( joypadData.BTN_SQUARE )// si bouton CARRE
							...
							restore = TTF_RenderText_Blended(font2, "REStORE SUCCESS", colorred); //défini le contenu de la variable restore
								{if(is_dev_blind_mounted()!=0); // verifie que dev_blind n'est pas monté
									...
									position.x = 250; //position horizontale de restore
						...
						if( joypadData.BTN_CROSS )// si bouton Croix
							...
							install = TTF_RenderText_Blended(font2, "INStall SUCCESS", colorred);//défini le contenu de la variable install
							nofiles = TTF_RenderText_Blended(font2, "FILES NOt FOUND", colorred);//défini le contenu de la variable install
							...
							{if(is_dev_blind_mounted()!=0); // verifie que dev_blind n'est pas monté
									...
									if(coldboot == NULL && coldboot2 == NULL && coldboot3 == NULL && crp == NULL && lines == NULL)// verifie si au moins un des fichiers installable est présent si il renvoi tous NULL on quitte
										...
										fclose(crp);
										fclose(lines);
									position.x = 250; //position horizontale de nofiles  et isntall???
									...
									sysLv2FsUnlink("/dev_hdd0/tmp/turnoff");// quitte le programme de la bonne manière
										...
										position.x = 250;//position horizontale de install
